...
import { getModes, getMode, reviewsForMode } from "@/lib/content";
...
export function generateStaticParams() {
  return getModes().map((m) => ({ mode: m.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ mode: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: absUrl(`/${m.slug}`) },
    openGraph: { title: m.title, description: m.description, url: absUrl(`/${m.slug}`) },
  };
}
      ...
      {m.key === "wheel" && (
        <section>
          ...
          <p className="mt-3 mb-4 max-w-2xl text-ash leading-relaxed">
            Enter your bet to see the win chance and payout for each multiplier on a standard Rust
            wheel. Use it to understand exactly what odds you are betting into before you spin.
          </p>
          <WheelCalculator />
        </section>
      )}
